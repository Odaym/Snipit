...
public class SnippetsActivity extends BaseActivity implements SearchView.OnQueryTextListener {
  ...
  @Bind(R.id.createNewSnippetBTN) FloatingActionsMenu createNewSnippetBTN;
  @Bind(R.id.emptyListLayout) RelativeLayout emptyListLayout;
  @Bind(R.id.snippetsList) DragSortListView listView;
  ...
  @Override protected void onSaveInstanceState(Bundle outState) {
    ...
    if (photoFile != null) outState.putString("photoFilePath", photoFile.getAbsolutePath());
  }
  ...
  @Override protected void onActivityResult(int requestCode, int resultCode, Intent intent) {
    ...
    Intent openCropSnippetIntent = new Intent(SnippetsActivity.this, CropImageActivity.class);
    openCropSnippetIntent.putExtra(Constants.EXTRAS_BOOK, book);

    switch (requestCode) {
      case REQUEST_CAPTURE_PERMISSIONS:
        openCropSnippetIntent.putExtra(Constants.EXTRAS_SNIPPET_TEMP_IMAGE_PATH,
  ...
  @Subscribe public void onBusEvents(EventBus_Poster ebp) {
    ...
    switch (ebpMessage) {
      ...
      case "snippet_image_updated":
        Helpers.delete_image_from_disk(ebp.getExtra());
        prepareForNotifyDataChanged(book.getId());
        snippetsAdapter.notifyDataSetChanged();
  ...
  public void showUndeleteDialog(final Snippet tempSnippetToDelete) {
    ...
    Spannable sentenceToSpan = new SpannableString(
        getResources().getString(R.string.delete_book_confirmation_message)
            + " "
            + tempSnippetToDelete.getName());
  ...
  public void finalizeSnippetDeletion(Snippet tempSnippet) {
    Helpers.delete_image_from_disk(tempSnippet.getImage_path());
  ...
  @Override public boolean onQueryTextChange(String searchQuery) {
    if (inSearchMode) {
  ...
  public class Snippets_Adapter extends BaseAdapter {
    ...
    public Snippets_Adapter(Context context) {
      ...
      this.roundedTransform = new RoundedTransform(
          context.getResources().getDimensionPixelSize(R.dimen.snippet_image_shape_corners_radius),
          context.getResources()
              .getDimensionPixelSize(R.dimen.snippet_image_shape_corners_padding_bottom));
    }
    ...
    @Override public View getView(final int position, View convertView, final ViewGroup parent) {
      ...
      if (convertView == null || ((SnippetsViewHolder) convertView.getTag()).needInflate) {
        ...
        holder.snippetActionLayout =
            (RelativeLayout) parentView.findViewById(R.id.snippetActionLayout);
        ...
        holder.imageProgressLoader =
            (ProgressBar) parentView.findViewById(R.id.imageProgressLoader);
        holder.needInflate = false;

        this.picassoImageLoadedCallback =
            new PicassoImageLoadedCallback(holder.imageProgressLoader) {
      ...
      if (android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
        LayerDrawable drawable =
            (LayerDrawable) ((LayerDrawable) holder.motherView.getBackground()).findDrawableByLayerId(
                R.id.content);
        gradient = (GradientDrawable) drawable.findDrawableByLayerId(R.id.innerView);
      } else {
        gradient =
            ((GradientDrawable) ((LayerDrawable) holder.motherView.getBackground()).findDrawableByLayerId(
                R.id.innerView));
      }
      ...
      if (snippets.get(position).getIsNoteShowing() == 0) {
        ...
        holder.snippetNoteBTN.setBackgroundDrawable(
      ...
      if (snippets.get(position).getPage_number() == Constants.NO_SNIPPET_PAGE_NUMBER) {
        ...
        holder.snippetPageNumber.setText(
            context.getResources().getText(R.string.snippet_page_number_label) + " " + snippets.get(
                position).getPage_number());
      }

      Picasso.with(SnippetsActivity.this)
          .load(new File(snippets.get(position).getImage_path()))
          .resize(context.getResources().getDimensionPixelSize(R.dimen.snippet_thumb_width),
              context.getResources().getDimensionPixelSize(R.dimen.snippet_thumb_height))
          .centerCrop()
