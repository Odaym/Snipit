...
public class DragSortListView extends ListView {
  ...
  public DragSortListView(Context context, AttributeSet attrs) {
    ...
    if (attrs != null) {
      ...
      mItemHeightCollapsed =
          Math.max(1, a.getDimensionPixelSize(R.styleable.DragSortListView_collapsed_height, 1));
      ...
      float frac =
          a.getFloat(R.styleable.DragSortListView_drag_scroll_start, mDragUpScrollStartFrac);

      setDragScrollStart(frac);

      mMaxScrollSpeed =
          a.getFloat(R.styleable.DragSortListView_max_drag_scroll_speed, mMaxScrollSpeed);

      removeAnimDuration =
          a.getInt(R.styleable.DragSortListView_remove_animation_duration, removeAnimDuration);

      dropAnimDuration =
          a.getInt(R.styleable.DragSortListView_drop_animation_duration, dropAnimDuration);
      ...
      if (useDefault) {
        ...
        int removeMode =
            a.getInt(R.styleable.DragSortListView_remove_mode, DragSortController.FLING_REMOVE);
        boolean sortEnabled = a.getBoolean(R.styleable.DragSortListView_sort_enabled, true);
        int dragInitMode =
            a.getInt(R.styleable.DragSortListView_drag_start_mode, DragSortController.ON_DOWN);
        ...
        int bgColor = a.getColor(R.styleable.DragSortListView_float_background_color, Color.GRAY);

        DragSortController controller =
            new DragSortController(this, dragHandleId, dragInitMode, removeMode, clickRemoveId,
                flingHandleId);
    ...
    mCancelEvent =
        MotionEvent.obtain(0, 0, MotionEvent.ACTION_CANCEL, 0f, 0f, 0f, 0f, 0, 0f, 0f, 0, 0);
  ...
  private static int findFirstSetIndex(SparseBooleanArray sba, int rangeStart, int rangeEnd) {
    ...
    while (i < size && sba.keyAt(i) < rangeEnd && !sba.valueAt(i)) i++;
  ...
  private void drawDivider(int expPosition, Canvas canvas) {
    ...
    // Helper_Methods.Log("mobeta", "div="+divider+" divH="+dividerHeight);
    ...
    if (divider != null && dividerHeight != 0) {
      ...
      if (expItem != null) {
        ...
        // Helper_Methods.Log("mobeta", "l="+l+" t="+t+" r="+r+" b="+b);
  ...
  @Override protected void dispatchDraw(Canvas canvas) {
    ...
    if (mFloatView != null) {
      ...
      canvas.save();
      // Helper_Methods.Log("mobeta", "clip rect bounds: " + canvas.getClipBounds());
   ...
   * and layoutChildren.
   *
   * @param top calculates this height.
  ...
  private boolean updatePositions() {
    ...
    // Helper_Methods.Log("mobeta", "float mid="+mFloatViewMid);

    int itemPos = startPos;
    int itemTop = startTop;
    if (mFloatViewMid < edge) {
      // scanning up for float position
      // Helper_Methods.Log("mobeta", "    edge="+edge);
      while (itemPos >= 0) {
        itemPos--;
    ...
    if (mAnimate) {
      ...
      // Helper_Methods.Log("mobeta", "edgeTop="+edgeTop+" edgeBot="+edgeBottom);
    ...
    if (mFirstExpPos != oldFirstExpPos
   ...
   * through to {@link com.bandindustries.roadie.DragSort_Listview.DragSortListView.DropListener#drop(int,
   * int)}.
   *
  ...
  private void adjustOnReorder() {
    ...
    // Helper_Methods.Log("mobeta", "first="+firstPos+" src="+mSrcPos);
    if (mSrcPos < firstPos) {
      ...
      // Helper_Methods.Log("mobeta", "top="+top+" fvh="+mFloatViewHeight);
  ...
  private void continueDrag(int x, int y) {
    ...
    } else if (maxY >= mUpScrollStartY
        && minY <= mDownScrollStartY
  ...
  private int getChildHeight(int position) {
    ...
    if (v != null) {
      ...
      if (childHeight != -1) {
        // Helper_Methods.Log("mobeta", "found child height in cache!");
        return childHeight;
      }
  ...
  private void measureItem(View item) {
    ...
    if (lp == null) {
      lp =
          new LayoutParams(ViewGroup.LayoutParams.FILL_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT);
  ...
  @Override protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    // Helper_Methods.Log("mobeta", "onMeasure called");
    if (mFloatView != null) {
  ...
  protected boolean onDragTouchEvent(MotionEvent ev) {
    ...
    switch (ev.getAction() & MotionEvent.ACTION_MASK) {
      case MotionEvent.ACTION_CANCEL:
        ...
        break;
      case MotionEvent.ACTION_UP:
        // Helper_Methods.Log("mobeta", "calling stopDrag from onDragTouchEvent");
        if (mDragState == DRAGGING) {
          stopDrag(false);
        }
        doActionUpOrCancel();
  ...
  private void updateFloatView() {
    ...
    // Helper_Methods.Log("mobeta", "dragView top=" + (y - mDragDeltaY));
    // Helper_Methods.Log("mobeta", "limit=" + limit);
    // Helper_Methods.Log("mobeta", "mDragDeltaY=" + mDragDeltaY);
   ...
   * {@link com.bandindustries.roadie.DragSort_Listview.DragSortListView.DragScrollProfile}.
   */
  ...
  public interface FloatViewManager {
     ...
     * and the offset provided in {@link com.bandindustries.roadie.DragSort_Listview.DragSortListView#startDrag}.
   ...
   * {@link com.bandindustries.roadie.DragSort_Listview.DragSortListView#setDragScrollProfile(com.bandindustries.roadie.DragSort_Listview.DragSortListView.DragScrollProfile)}
   * to
   * set custom profile.
   *
   * @author heycosmo
  ...
  private class LiftAnimator extends SmoothAnimator {
    ...
    @Override public void onUpdate(float frac, float smoothFrac) {
      if (mDragState != DRAGGING) {
        cancel();
      } else {
  ...
  private class DragScroller implements Runnable {
    ...
    @Override public void run() {
      ...
      if (scrollDir == UP) {
        ...
        mScrollSpeed =
            mScrollProfile.getSpeed((mUpScrollStartYF - maxY) / mDragUpScrollHeight, mPrevTime);
      } else {
