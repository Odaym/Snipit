...
public class DragSortController extends SimpleFloatViewManager
  ...
  private GestureDetector.OnGestureListener mFlingRemoveListener =
      new GestureDetector.SimpleOnGestureListener() {
        @Override public final boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
            float velocityY) {
          // Helper_Methods.Log("mobeta", "on fling remove called");
  ...
  public int viewIdHitPosition(MotionEvent ev, int id) {
    ...
    // Helper_Methods.Log("mobeta", "touch down on position " + itemnum);
    // We're only interested if the touch was on an
    // item that's not a header or footer.
    if (touchPos != AdapterView.INVALID_POSITION && touchPos >= numHeaders && touchPos < (count
        - numFooters)) {
  ...
  @Override public void onLongPress(MotionEvent e) {
    // Helper_Methods.Log("mobeta", "lift listener long pressed");
